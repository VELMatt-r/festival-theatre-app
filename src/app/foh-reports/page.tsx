...
export default function FOHReportsPage() {
  const [reports, setReports] = useState<FOHReport[]>([]);
  ...
  async function loadReports() {
    ...
    if (error) {
      console.error("Load FOH reports failed:", JSON.stringify(error, null, 2));
      alert(error.message || "Failed to load FOH reports.");
      return;
    }
  ...
  const filteredReports = useMemo(() => {
    return reports.filter((report) => {
      ...
      const matchesSearch =
        report.show_name?.toLowerCase().includes(searchValue) ||
        report.venue_name?.toLowerCase().includes(searchValue);
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 text-zinc-400">
              Draft and submitted Front of House reports.
            </p>
          </div>

          <Link
            href="/foh-reports/new"
        ...
        <section className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
          <div className="grid gap-4 md:grid-cols-2">
            <input
              type="text"
              placeholder="Search FOH reports..."
              value={search}
