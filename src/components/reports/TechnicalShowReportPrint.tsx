...
type TechnicalShowReportPrintProps = {
    reportId: number;
};

export default function TechnicalShowReportPrint({
  reportId,
}: TechnicalShowReportPrintProps) {
  ...
  useEffect(() => {
    async function loadReport() {
      ...
      if (error) {
        console.error("Load print report failed:", error);
        return;
      }
      ...
      <div className="mx-auto max-w-5xl overflow-hidden rounded-3xl bg-white shadow-2xl print:max-w-none print:rounded-none print:shadow-none">
        ...
        <div className="space-y-8 px-10 py-8 print:px-8">
            ...
            {!requiresOpeningChecks ? (
              ...
              <>
                ...
                <PrintRow
                  ...
                  value={yesNo(report.dressing_rooms_set_up)}
                  booleanValue={report.dressing_rooms_set_up}
                />

                <PrintRow
                  label="Additional Equipment on Site"
                  value={yesNo(report.additional_equipment_on_site)}
                  booleanValue={report.additional_equipment_on_site}
                />
          ...
          <PrintSection title="Act 1">
            ...
            <PrintSubheading>Sound Levels</PrintSubheading>

            <PrintRow
              label="FOH"
              value={formatSoundLevel(report.act_1_sound_foh)}
            />
            <PrintRow
              label="Box Office"
              value={formatSoundLevel(
                report.act_1_sound_box_office
              )}
            />
            <PrintRow
              label="Lake"
              value={formatSoundLevel(report.act_1_sound_lake)}
            />
            <PrintRow
              label="Cottages"
              value={formatSoundLevel(
                report.act_1_sound_cottages
              )}
            />
            <PrintRow
              label="Top Gate"
              value={formatSoundLevel(
                report.act_1_sound_top_gate
              )}
            />
          </PrintSection>

          <PrintSection title="Act 2">
            <PrintRow
              label="Clearance Time"
            ...
            <PrintRow
              label="FOH"
              value={formatSoundLevel(report.act_2_sound_foh)}
            />
            <PrintRow
              label="Box Office"
              value={formatSoundLevel(
                report.act_2_sound_box_office
              )}
            />
            <PrintRow
              label="Lake"
              value={formatSoundLevel(report.act_2_sound_lake)}
            />
            <PrintRow
              label="Cottages"
              value={formatSoundLevel(
                report.act_2_sound_cottages
              )}
            />
            <PrintRow
              label="Top Gate"
              value={formatSoundLevel(
                report.act_2_sound_top_gate
              )}
            />
          </PrintSection>
            ...
            {!requiresOpeningChecks ? (
              ...
              <>
                ...
                {report.sides_closed === false && (
                  <>
                    ...
                    <PrintRow
                      ...
                      value={yesNo(report.barriers_in_place)}
                      booleanValue={report.barriers_in_place}
                    />
                  </>
                )}
                ...
                <PrintRow
                  ...
                  value={yesNo(report.new_structure_damage)}
                  booleanValue={report.new_structure_damage}
                />
          ...
          <PrintSection title="Submission">
            ...
            <PrintRow
              label="Submitted By"
              value={report.submitted_by_name || "Unknown"}
            />
            <PrintRow
              label="Submitted At"
              value={
...
function PrintSubheading({
  ...
  return (
    <h3 className="pt-3 text-xs font-black uppercase tracking-[0.18em] text-zinc-500">
      {children}
    </h3>
  );
}
